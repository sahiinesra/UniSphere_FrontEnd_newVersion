...
const api = axios.create({
  baseURL: 'http://192.168.1.136:8080/api/v1',
...
const Profile = () => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        const photoUrl = profileData.profilePhotoUrl ? 
          profileData.profilePhotoUrl.replace('http://localhost:8080', 'http://192.168.1.136:8080') : 
          null;
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        const response = await axios.post(
          'http://192.168.1.136:8080/api/v1/users/profile/photo',
          formData,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
  ...
  const handleDeleteProfilePhoto = async () => {
    try {
      ...
      await axios.delete('http://192.168.1.136:8080/api/v1/users/profile/photo', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
        },
